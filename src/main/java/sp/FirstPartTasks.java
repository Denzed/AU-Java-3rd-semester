...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
    	return albums.collect(Collectors.groupingBy((x) -> x, 
    												Collectors.counting()))
    			     .entrySet()
    				 .stream()
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min(Comparator
        		.comparingInt((album) -> album.getTracks()
        									  .stream()
        									  .map(Track::getRating)
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
    	ToDoubleFunction<Album> avgRating = (album) -> album.getTracks()
    														.stream()
    														.collect(Collectors.averagingInt(
    																Track::getRating));
        return albums.sorted(Comparator.comparingDouble(avgRating)
        							   .reversed())
        			 .collect(Collectors.toList());
    }
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream.reduce(1, (pref,cur) -> pref * cur % modulo);
    }
    ...
    public static <R> Stream<R> filterIsInstance(Stream<?> s, Class<R> clazz) {
    	return s.flatMap((elem) -> {
    		Stream.Builder<R> res = Stream.builder();
			if (clazz.isInstance(elem)) {
				res.accept(clazz.cast(elem));
			}
			return res.build();
    	});
    }
